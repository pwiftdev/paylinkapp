...
// Try to load InertiaPlugin if available; fall back gracefully if not
let inertiaAvailable = false;
(async () => {
  try {
    const mod: any = await import('gsap/InertiaPlugin');
    if (mod && mod.InertiaPlugin) {
...
const DotGrid: React.FC<DotGridProps> = ({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const dot of dotsRef.current) {
        ...
        ctx.fillStyle = style;
        if ((ctx as any).fill) {
          (ctx as any).fill(circlePath as any);
        }
        ctx.restore();
      }
      ...
      for (const dot of dotsRef.current) {
        ...
        if (speed > speedTrigger && dist < proximity && !dot._inertiaApplied) {
          ...
          if (inertiaAvailable) {
            ...
            // Fallback without InertiaPlugin
            gsap.to(dot, {
              xOffset: pushX,
              yOffset: pushY,
              duration: Math.min(0.4 + (Math.abs(pushX) + Math.abs(pushY)) / 1000, 1.2),
              ease: 'power2.out',
              onComplete: () => {
                gsap.to(dot, {
                  xOffset: 0,
  ...
  return (
    <section className={`p-0 m-0 h-full w-full relative ${className}`} style={style}>
